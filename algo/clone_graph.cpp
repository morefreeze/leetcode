        ...
        UndirectedGraphNode *cloneGraph(UndirectedGraphNode *node) {
            ...
            while (!q.empty()) {
                ...
                for (int i = 0;i < int(cur->neighbors.size()); ++i) {
                    if (m.find(cur->label) == m.end()) {
                        m[cur->label] = new UndirectedGraphNode(cur->label);
                    }
                    if (m.find(cur->neighbors[i]->label) == m.end()) {
                        m[cur->neighbors[i]->label] = new UndirectedGraphNode(
                                cur->neighbors[i]->label);
                        q.push_back(cur->neighbors[i]);
                    }
...
#endif /* end of include guard: CLONE_GRAPH_H */
